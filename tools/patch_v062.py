...
# WakeGuard targets API 36, so Activity PendingIntents used by AlarmManager and
# full-screen notifications must explicitly opt in to background Activity launch.

service = service_path.read_text(encoding="utf-8")
if "import android.app.ActivityOptions;" not in service:
    service = service.replace("import android.app.*;", "import android.app.*;", 1) if "import android.app.*;" in service else service.replace("import android.app.AlarmManager;", "import android.app.AlarmManager;\nimport android.app.ActivityOptions;", 1)
...
scheduler = scheduler.replace('''        return PendingIntent.getActivity(c, requestCode(date) + 400000, i,\n                flags | PendingIntent.FLAG_IMMUTABLE);\n''','''        return activityPi(c, requestCode(date) + 400000, i, flags);\n''',1)
scheduler = scheduler.replace('''        return PendingIntent.getActivity(c, requestCode(date), i,\n                flags | PendingIntent.FLAG_IMMUTABLE);\n''','''        return activityPi(c, requestCode(date), i, flags);\n''',1)
scheduler = scheduler.replace('''                PendingIntent showPi = PendingIntent.getActivity(c, requestCode(d) + 100000,\n                        show, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);\n''','''                PendingIntent showPi = activityPi(c, requestCode(d) + 100000,\n                        show, PendingIntent.FLAG_UPDATE_CURRENT);\n''',1)
scheduler = scheduler.replace('''            PendingIntent showPi = PendingIntent.getActivity(c, RECOVERY_REQUEST + 1, show,\n                    PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);\n''','''            PendingIntent showPi = activityPi(c, RECOVERY_REQUEST + 1, show,\n                    PendingIntent.FLAG_UPDATE_CURRENT);\n''',1)
scheduler_path.write_text(scheduler, encoding="utf-8")

activity = activity_path.read_text(encoding="utf-8")
if "import android.app.PendingIntent;" not in activity and "import android.app.*;" not in activity:
    activity = activity.replace("package jp.wakeguard.alarm;\n", "package jp.wakeguard.alarm;\n\nimport android.app.PendingIntent;", 1)
if "import android.app.ActivityOptions;" not in activity and "import android.app.*;" not in activity:
    activity = activity.replace("import android.app.PendingIntent;", "import android.app.PendingIntent;\nimport android.app.ActivityOptions;", 1)
