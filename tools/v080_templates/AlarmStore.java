...
public final class AlarmStore {
    ...
    public static final class Entry {
        public long id; public int hour,minute,dayMask,steps,volume; public boolean enabled;
        public String label,vibration,soundUri,soundName; public long soundBytes,soundDurationMs;
        public Entry(long id,int hour,int minute,int dayMask,boolean enabled,String label,
                     int steps,int volume,String vibration,String soundUri,String soundName,long soundBytes,long soundDurationMs) {
            this.id=id;this.hour=hour;this.minute=minute;this.dayMask=dayMask;this.enabled=enabled;this.label=label==null?"":label;
            this.steps=Math.max(1,Math.min(9999,steps));this.volume=Math.max(0,Math.min(100,volume));
    ...
    private static Entry read(Context c,JSONObject o){
        ...
        return new Entry(id,Math.max(0,Math.min(23,o.optInt("hour",7))),Math.max(0,Math.min(59,o.optInt("minute",0))),o.optInt("days",0)&0x7F,
                ...
                o.has("steps")?o.optInt("steps",50):Prefs.steps(c),o.has("volume")?o.optInt("volume",15):Prefs.volume(c),
    ...
    private static JSONObject json(Entry e)throws Exception{
        ...
        o.put("steps",e.steps);o.put("volume",e.volume);o.put("vibration",e.vibration);o.put("soundUri",e.soundUri);o.put("soundName",e.soundName);o.put("soundBytes",e.soundBytes);o.put("soundDurationMs",e.soundDurationMs);return o;
    }
    ...
    public static synchronized Entry add(Context c,Entry draft){
        ...
        Entry e=new Entry(id,draft.hour,draft.minute,draft.dayMask,draft.enabled,draft.label,draft.steps,draft.volume,draft.vibration,draft.soundUri,draft.soundName,draft.soundBytes,draft.soundDurationMs);
